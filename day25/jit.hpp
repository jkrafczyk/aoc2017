...
namespace day25 {
    ...
    struct Symbol {
        Symbol(const std::string &name): name(name), offset(-1), address(nullptr) {}
        Symbol(const std::string &name, uint32_t offset, void *address): name(name), offset(offset), address(address) {}
