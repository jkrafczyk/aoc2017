...
namespace day25 {
    BytecodeExecutor::BytecodeExecutor(Program program): m_program(program) {
        ...
        if (m_program.states.size() > 32) {
            throw new std::runtime_error("Bytecode interpreter only works with up to 32 states!");
        }
    ...
    void BytecodeExecutor::step() {
        ...
        m_memory_offset = (m_memory_offset + m_program.checksum_delay + move_direction) % (m_program.checksum_delay);
        m_state = next_state;
    }
    ...
    uint16_t BytecodeExecutor::encode_state(const day25::State &state) {
        ...
        return encode_action(state.actions.at(1)) << 8 | encode_action(state.actions.at(0));
    }
    ...
    uint8_t BytecodeExecutor::encode_action(const day25::StateAction &action) {
        ...
        if (write_contents != action.write_value || move_direction != action.move_direction || next_state != m_state_map.at(action.next_state)) {
            throw new std::runtime_error("Bug: decoding instruction does not yield original encoder input.");
        }
        return result;
    }

    void BytecodeExecutor::decode_action(uint8_t& encoded, uint8_t &write_contents, int8_t &move_direction, uint8_t &next_state) {
