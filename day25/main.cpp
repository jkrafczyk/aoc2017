...
int generate_c(Program program, ostream &cfile) {
  ...
  cfile << "  for (unsigned long steps = 0; steps < " << program.checksum_delay
        << "; steps++) {" << endl;
  ...
  cfile << R"(  printf("Total executed steps: )" << program.checksum_delay * 25
        << R"(\n");)" << endl;
...
int benchmark(Program program, const string &executor_name,
  ...
  if (executor_name == "") {
    ...
    cout << indent << "Benchmarking with executor " << executor_name << " for "
         << target_seconds << " seconds." << endl;
    auto executor = get_executor(executor_name, program);
