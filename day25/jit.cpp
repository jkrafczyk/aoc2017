...
void Jit::finalize_code() {
    ...
    for (auto ref: m_symbol_refs) {
        ...
        int64_t distance = (uint8_t*)ref_addr - ref.replacement_length - (uint8_t*)here;
        if (ref.absolute) {
            memcpy(here, &ref_addr, sizeof(void *));
        } else if (!ref.absolute && ref.replacement_length == 1) {
            if (distance < -128 || distance > 127) {
                throw runtime_error("Relative reference to symbol too far away.");
            }
            int8_t i = distance;
            memcpy(here, &i, 1);
            ...
            throw runtime_error("Invalid symbol reference: Relative reference with replacement size that is not 1, 2 or 4 bytes.");
        }
    }

    //TODO: Do relocations, fix jumps, calls and moves, etc. based on symbol table.
...
void Jit::emit_lea(Register reg, Indirect addr) {
    emit(rex(1, reg >= Register::R8, addr.offset_reg >= Register::R8 && addr.offset_reg <= Register::NONE, addr.reg >= Register::R8));
    emit((uint8_t)0x8d);
    emit(ModR(0, reg, Register::RSP));
    auto index = (uint8_t)((addr.offset_reg == Register::NONE) ? Register::RSP : addr.offset_reg)&0x7;
...
uint64_t Jit::call(uint64_t arg) {
    return call(m_code, arg);
}
