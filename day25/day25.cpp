...
Arguments parse_args(int argc, char **argv) {
    ...
    for (int i=1; i<argc; i++) {
        ...
        if (i == 1) {
            ...
            result.program = arg;
        } else if (i == 3 && (result.action == Arguments::RUN || result.action == Arguments::BENCHMARK)) {
            auto executors = list_executors();
            if (find(executors.begin(), executors.end(), arg) != executors.end()) {
                result.executor = arg;
            } else {
...
int generate_c(Program program, ostream &cfile) {
    ...
    cfile << "void run() {" << endl;
    cfile << "  unsigned current_state = " << state_map[program.initial_state] << ";" << endl;
    cfile << "  unsigned long current_offset = 0;" << endl;
    cfile << "  for (unsigned long steps = 0; steps < " << program.checksum_delay << "; steps++) {" << endl;
    cfile << "    switch(current_state) {" << endl;
    for (auto state: program.states) {
        cfile << "      case " << state_map[state.first] << ":" << endl;
        cfile << "        if (tape[current_offset] == 0) {" << endl;
        cfile << "          tape[current_offset] = " << state.second.actions[0].write_value << ";" << endl;
        cfile << "          current_offset = (current_offset + sizeof(tape) + " << state.second.actions[0].move_direction << ") % sizeof(tape);" << endl;
        cfile << "          current_state = " << state_map[state.second.actions[0].next_state] << ";" << endl;
        cfile << "        } else {" << endl;
        cfile << "          tape[current_offset] = " << state.second.actions[1].write_value << ";" << endl;
        cfile << "          current_offset = (current_offset + sizeof(tape) + " << state.second.actions[1].move_direction << ") % sizeof(tape);" << endl;
        cfile << "          current_state = " << state_map[state.second.actions[1].next_state] << ";" << endl;
        cfile << "        }" << endl;
        cfile << "        break;" << endl;
    }
    cfile << "    } //switch" << endl;
    ...
    cfile << R"(  printf("Total executed steps: )" << program.checksum_delay * 25 << R"(\n");)" << endl;
    cfile << "  printf(\"%lf steps/ms\\n%lf us/steps\\n\", " << program.checksum_delay * 25 << "/ duration, 1000 * duration / " << program.checksum_delay * 25 << ");" << endl;
...
int benchmark(Program program, const string &executor_name, const string indent = "") {
    ...
    if (executor_name == "") {
        ...
        cout << indent << "Benchmarking with executor " << executor_name << " for " << target_seconds << " seconds." << endl;
        auto executor = get_executor(executor_name, program);
        ...
        duration *= 1000;
        duration /= CLOCKS_PER_SEC;
        cout << indent << "Took " << duration << "ms for " << step_count << " steps." << endl
             << indent << "  " << step_count / duration << " steps/ms" << endl
             << indent << "  " << 1000 * duration / step_count << " us/step" << endl;
        return 0;
    }
}
...
int main(int argc, char **argv) {
    ...
    if (args.action == Arguments::NONE) {
        ...
        return usage(argv[0]);
    } else if (args.action == Arguments::RUN && (args.program.empty() || (args.executor.empty()))) {
        return usage(argv[0]);
    }
