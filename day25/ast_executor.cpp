...
namespace day25 {
    AstExecutor::AstExecutor(Program program): m_program(program), m_memory(program.checksum_delay), m_offset(0), m_state(program.initial_state) {
    }
    ...
    void AstExecutor::step() {
        ...
        if (action.move_direction < 0 && m_offset == 0) {
            ...
            m_offset = (m_offset + action.move_direction) % m_program.checksum_delay;
        }
